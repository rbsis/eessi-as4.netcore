...
public abstract class PollingTemplate<TIn, TOut> where TIn : class
{
    protected readonly ILogger<PollingTemplate<TIn, TOut>> _logger;

    protected PollingTemplate(ILogger<PollingTemplate<TIn, TOut>> logger)
    {
        _logger = logger;
    }
    ...
    protected void StartPolling(Func<TOut, CancellationToken, Task<MessagingContext>> onMessage, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var messagesToPoll = GetMessagesToPoll(cancellationToken);

                if (messagesToPoll?.Any() == true)
                {
                    var taskCollection = CreateTaskForEachMessage(messagesToPoll, onMessage, cancellationToken);
        ...
        {
            _logger.LogTrace(ex, "Cancellation requested; stopped polling.");
        }
        finally
        {
    ...
    private void WaitForAllTasksToComplete(IEnumerable<Task> taskCollection, IEnumerable<TIn> messagesToPoll)
    {
        ...
        {
            foreach (var message in messagesToPoll)
            {
    ...
    protected abstract Task MessageReceivedAsync(
        TIn entity,
        ...
        CancellationToken cancellationToken)
    {
        return messagesToPoll
            .Where(m => m != null)
            .Select(message =>
                Task.Run(() => MessageReceivedAsync(message, messageCallback, cancellationToken))
    ...
    private void LogInnerExceptions(Task task)
    {
        ...
        {
            _logger.LogCritical(ex, "Inner exception");
        }
    }
}
