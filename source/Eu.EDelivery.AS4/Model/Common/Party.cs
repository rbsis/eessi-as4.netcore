...
public class Party : IEquatable<Party>, IEquatable<PMode.Party>
{
    ...
    {
        IEnumerable<string> submitPartyIds = (PartyIds ?? Enumerable.Empty<PartyId>())
            .Select(p => p.Id)
            .OrderBy(id => id, StringComparer.OrdinalIgnoreCase);

        IEnumerable<string> pmodePartyIds = (pmodeParty?.PartyIds ?? Enumerable.Empty<PMode.PartyId>())
            .Where(p => !p.IsEmpty())
            .Select(p => p.Id!)
            .OrderBy(id => id, StringComparer.OrdinalIgnoreCase);
