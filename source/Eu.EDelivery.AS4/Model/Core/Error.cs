...
public class Error : SignalMessage
{
    ...
    public Error(string messageId, string? refToMessageId, ErrorLine line)
        : this(messageId, refToMessageId, [line]) { }
...
public sealed class ErrorLine : IEquatable<ErrorLine>
{
    ...
    public static ErrorLine FromErrorResult(ErrorResult result)
    {
        ...
        var category = ErrorCodeUtils.GetCategory(result.Code);
    ...
    {
        return HashCode.Combine(ErrorCode, Severity, Origin, Category, RefToMessageInError, ShortDescription, Description, Detail);
    }
    ...
    public static bool operator ==(ErrorLine left, ErrorLine right) => Equals(left, right);
...
public sealed class ErrorDescription : IEquatable<ErrorDescription>
{
    ...
    /// <param name="right">The second value to compare.</param>
    /// <returns>true if the <paramref name="left" /> and <paramref name="right" /> parameters have the same value; otherwise, false.</returns>
    public static bool operator ==(ErrorDescription left, ErrorDescription right) => Equals(left, right);
