...
public class Receipt : SignalMessage
{
        ...
        bool userMessageSendViaMultiHop = false)
    {
        ...
        {
            var signedReferences = userMessageSecurityHeader.GetReferences();
    ...
    {
        byte[]? GetNonRepudiationHashForUri(string userMessageReferenceUri) => NonRepudiationInformation?
            .MessagePartNRIReferences
            .Where(r => userMessageReferenceUri.Equals(r.URI))
        ...
        bool IsNonRepudiationHashEqualToUserReferenceHash(CryptoReference r)
        {
            ...
            var repudiationHash = GetNonRepudiationHashForUri(r.Uri);
            return repudiationHash != null && r.DigestValue?.SequenceEqual(repudiationHash) == true;
        }

        var userReferences = userMessage.SecurityHeader?.GetReferences();

        return userReferences != null && userReferences.Any()
