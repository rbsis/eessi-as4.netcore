...
internal static partial class PayloadFileNameFactory
{
    private static readonly Regex MacroMatchRegex = MyRegex();

    private static readonly Dictionary<string, Func<Attachment, MessageInfo, string?>> _namingMacros
    ...
    public static string CreateFileName(string? pattern, Attachment attachment, MessageInfo userMessage)
    {
        ...
        {
            if (!_namingMacros.Keys.Any(p => pattern.IndexOf(p, StringComparison.OrdinalIgnoreCase) > -1))
            {
    ...
    private static StringBuilder ReplaceValueWithMacro(StringBuilder idBuilder, Match match, Attachment attachment, MessageInfo userMessage)
    {
        ...
        if (_namingMacros.TryGetValue(macroName, out var value))
        {
            idBuilder = idBuilder.Replace(valueToReplace, value(attachment, userMessage));
        }
