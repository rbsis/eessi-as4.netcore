...
internal abstract class SignatureStrategy : SignedXml
{
    ...
    private static readonly FieldInfo? _refTargetTypeField = typeof(Reference).GetField("m_refTargetType", BindingFlags.Instance | BindingFlags.NonPublic);
    private static readonly FieldInfo? _refTargetField = typeof(Reference).GetField("m_refTarget", BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    protected override AsymmetricAlgorithm? GetPublicKey()
    {
        ...
        {
            publicKey = signingCertificate.GetRSAPublicKey();
        }
    ...
    protected static void SetReferenceStream(Reference reference, Attachment attachment)
    {
        ...
        _refTargetTypeField?.SetValue(reference, 0);

        _refTargetField?.SetValue(reference, new NonCloseableStream(attachment.Content));
    }
    ...
    {
        var nodeSignature = _soapEnvelope.SelectSingleNode("//*[local-name()='Signature'] ");
