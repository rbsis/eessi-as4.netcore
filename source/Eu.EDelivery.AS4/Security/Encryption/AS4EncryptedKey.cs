...
internal class AS4EncryptedKey
{
    ...
    internal class EncryptedKeyBuilder
    {
        ...
        public AS4EncryptedKey Build() => new(
            BuildEncryptedKey(),
        ...
        private EncryptedKey BuildEncryptedKey()
        {
            if (_key == null)
            {
                throw new InvalidOperationException("AS4EncryptedKey already build.");
            }

            var encoding = EncodingFactory.Create(
                digestAlgorithm: _keyEncryptionConfiguration.DigestMethod,
                mgfAlgorithm: _keyEncryptionConfiguration.Mgf);

            var rsaPublicKey = _keyEncryptionConfiguration.EncryptionCertificate.GetRSAPublicKey();
            var publicKey = DotNetUtilities.GetRsaPublicKey(rsaPublicKey);
            encoding.Init(forEncryption: true, parameters: publicKey);
    ...
    {
        var encryptedKeyElement = xmlDocument.SelectSingleNode("//*[local-name()='EncryptedKey']") as XmlElement ?? throw new ArgumentException(@"No EncryptedKey element found in XmlDocument", nameof(xmlDocument));
        var encryptedKey = new EncryptedKey();
    ...
    {
        var xpath = $".//*[local-name()='EncryptionMethod']/*[local-name()='DigestMethod' and namespace-uri()='{Constants.Namespaces.XmlDsig}']";
    ...
    {
        var xpath = ".//*[local-name()='EncryptionMethod']/*[local-name()='MGF']";
    ...
    {
        var xpath = ".//*[local-name()='EncryptionMethod']";
    ...
    {
        var encryptedKeyElement = GetEncryptedKeyElement();
        var importedEncryptedKeyNode = securityElement.OwnerDocument.ImportNode(encryptedKeyElement, deep: true);
    ...
    {
        var encryptedKeyElement = _encryptedKey.GetXml();
    ...
    {
        if (encryptionMethodNode.OwnerDocument == null)
        {
            return;
        }

        var digestMethod = encryptionMethodNode.OwnerDocument.CreateElement("DigestMethod", Constants.Namespaces.XmlDsig);
