...
public class ExceptionService : IExceptionService
{
    ...
    {
        var stream = await AS4XmlSerializer.ToStreamAsync(submit, cancellation);
        var location = await _bodyStore.SaveAS4MessageStreamAsync(
            _config.InExceptionStoreLocation,
            stream, cancellation);

        var entity = InException
            .ForMessageBody(messageLocation: location, exception: exception)
            .SetOperationFor(pmode?.ExceptionHandling);

        await entity.SetPModeInformationAsync(pmode, cancellation);
    ...
    /// <param name="cancellation"></param>
    public async Task<InException> InsertIncomingAS4MessageExceptionAsync(Exception exception, string? ebmsMessageId, ReceivingProcessingMode? pmode, CancellationToken cancellation)
    {
        var entity = InException
    ...
    public async Task<OutException> InsertOutgoingAS4MessageExceptionAsync(
        ...
        long? entityId,
        SendingProcessingMode? pmode, CancellationToken cancellation)
    {
        var entity = OutException
            .ForEbmsMessageId(ebmsMessageId, exception)
            .SetOperationFor(pmode?.ExceptionHandling);
    ...
    /// <param name="cancellation"></param>
    public async Task InsertIncomingExceptionAsync(Exception exception, Stream messageStream, CancellationToken cancellation)
    {
        var location = await _bodyStore.SaveAS4MessageStreamAsync(
            _config.InExceptionStoreLocation,
            messageStream, cancellation);

        var entity = InException.ForMessageBody(location, exception);
        _repository.InsertInException(entity);
    }
    ...
    /// <param name="messageStream">The stream that represents the message which caused the exception.</param>
    /// <param name="cancellation"></param>
    public async Task InsertOutgoingExceptionAsync(Exception exception, Stream messageStream, CancellationToken cancellation)
    {
        var location = await _bodyStore.SaveAS4MessageStreamAsync(
            _config.OutExceptionStoreLocation,
            messageStream, cancellation);

        var entity = OutException.ForMessageBody(location, exception);
        _repository.InsertOutException(entity);
    }
}
