...
internal class InMessageBuilder
{
    ...
    public static InMessageBuilder ForUserMessage(UserMessage userMessage, AS4Message belongsToAS4Message, MessageExchangePattern mep) =>
        new(userMessage, belongsToAS4Message.ContentType, mep);
    ...
    public static InMessageBuilder ForSignalMessage(SignalMessage signalMessage, AS4Message belongsToAS4Message, MessageExchangePattern mep)
        => new(signalMessage, belongsToAS4Message.ContentType, mep);
    ...
    {
        var inMessage = BuildAsToBeProcessed();
        inMessage.Operation = (_pmode?.ErrorHandling?.NotifyMessageProducer ?? false)
    ...
    private static MessageType DetermineMessageType(MessageUnit messageUnit) => messageUnit switch
    {
        UserMessage _ => MessageType.UserMessage,
        Receipt _ => MessageType.Receipt,
        Error _ => MessageType.Error,
        _ => throw new InvalidOperationException("There is no MessageType mapped for this MessageUnit."),
    };
}
