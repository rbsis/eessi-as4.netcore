...
public class ExceptionEntity : Entity
{
    ...
    public async Task SetPModeInformationAsync(IPMode? pmode, CancellationToken cancellation)
    {
        if (pmode != null)
        {
            PModeId = pmode.Id;
            ...
            {
                PMode = await AS4XmlSerializer.ToStringAsync(sp, cancellation);
            }
            else if (pmode is ReceivingProcessingMode rp)
            {
                PMode = await AS4XmlSerializer.ToStringAsync(rp, cancellation);
            }
            else
            {
    ...
    /// </summary>
    public virtual SendingProcessingMode? GetSendingPMode() => null;

    /// <summary>
    /// Gets the receiving processing mode of the child representation of an exception.
    /// </summary>
    /// <returns></returns>
    public virtual ReceivingProcessingMode? GetReceivingPMode() => null;
}
