...
{
    private static readonly Dictionary<string, Func<string?, DbContextOptionsBuilder, DbContextOptionsBuilder>> _dbProviders = new(StringComparer.InvariantCulture)
    {
        ["Sqlite"] = (c, b) =>
        {
            static string? GetDirectoryFromConnectionString(string? connectionString)
            {
            ...
            var databaseLocation = GetDirectoryFromConnectionString(c);
    ...
    private static readonly Dictionary<string, Func<DatastoreContext, LoggerFactory, IAS4DbCommand>> _nativeCommandsProvider = new()
    {
        ["SqlServer"] = (ctx, loggerFactory) => new SqlServerDbCommand(ctx, loggerFactory.CreateLogger<SqlServerDbCommand>()),
        ["Sqlite"] = (ctx, loggerFactory) => new SqliteDbCommand(ctx, loggerFactory.CreateLogger<SqliteDbCommand>()),
        ["InMemory"] = (ctx, loggerFactory) => new InMemoryDbCommand(ctx)
    };

    private readonly ILogger<DatastoreContext> _logger;
    ...
    public DatastoreContext(
        ILogger<DatastoreContext> logger,
        IConfig config,
        DbContextOptions<DatastoreContext> options) : base(options)
    {
        _logger = logger;
        _config = config;
        _policy = Policy
        ...
        var providerKey = _config.DatabaseProvider;
        if (providerKey == null || !_nativeCommandsProvider.TryGetValue(providerKey, out var value))
        {
            throw new KeyNotFoundException($"No Native Command implementation found for DBMS-type: '{providerKey}'");
        }

        NativeCommands = value(this, _loggerFactory);
    }
    ...
    /// <returns></returns>
    public async Task<TResult> TransactionalAsync<TResult>(Func<DatastoreContext, Task<TResult>> f, CancellationToken cancellation)
    {
    ...
    /// <param name="cancellation"></param>
    /// <returns></returns>
    public async Task TransactionalAsync(Func<DatastoreContext, Task> f, CancellationToken cancellation)
    {
        if (Database.CurrentTransaction != null)
        {
            await f(this);
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        var providerKey = _config.DatabaseProvider;
        if (providerKey == null || !_dbProviders.TryGetValue(providerKey, out var databaseInitializer))
        {
        ...
        //Obsolete: optionsBuilder.ConfigureWarnings(x => x.Ignore(CoreEventId.IncludeIgnoredWarning))
    ...
    private static readonly LoggerFactory _loggerFactory = new([new TraceLoggerProvider()]);
    ...
    private DataException ThrowDatastoreUnavailableException(Exception? innerException = null)
    {
        Exception? mostInnerException = null;

        while (innerException != null)
        {
            _logger.LogError(innerException, "Datastore unavailable");
