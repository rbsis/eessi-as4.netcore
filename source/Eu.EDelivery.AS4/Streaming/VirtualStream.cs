...
public sealed class VirtualStream : Stream, ICloneable
{
    ...
    public static VirtualStream Create(bool forAsync = false) => Create(ThresholdMax, forAsync);
    ...
    private static Stream CreatePersistentStream(bool forAsync)
    {
        ...
        var options = FileOptions.DeleteOnClose | FileOptions.SequentialScan;
    ...
    {
        ThrowIfDisposed();
        if (_memoryStatus == MemoryFlag.AutoOverFlowToDisk && _isInMemory && value > _threshholdSize)
        {
    ...
    {
        var persistentStream = CreatePersistentStream(_forAsync);
    ...
    public byte[] ToArray()
    {
        var bytes = GetUnderlyingBytes();
        UnderlyingStream.MovePositionToStreamStart();
