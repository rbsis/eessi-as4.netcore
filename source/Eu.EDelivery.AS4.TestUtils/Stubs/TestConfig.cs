...
public class TestConfig : IConfig
{
    ...
    public TestConfig(string provider, string connectionString, string storeLocation)
    {
        ...
        var basePath = storeLocation?.TrimEnd('\\') ?? "file:///.\\database";
    ...
    public static IConfig Create(Settings settings)
    {
        ...
        ArgumentNullException.ThrowIfNull(settings.Database.Provider);
        ArgumentNullException.ThrowIfNull(settings.Database.ConnectionString);
        ArgumentNullException.ThrowIfNull(settings.Database.StoreLocation);
    ...
    {
        var pmodePath = Path.Combine(Environment.CurrentDirectory, "config", "send-pmodes");
    ...
    {
        var pmodePath = Path.Combine(Environment.CurrentDirectory, "config", "receive-pmodes");
        return Directory.GetFiles(pmodePath, "*.xml")
                        .Select(File.ReadAllText)
    ...
    public bool PayloadServiceInProcess => throw new NotImplementedException();

    public bool FeInProcess => throw new NotImplementedException();
}
