...
public class PayloadController : Controller
{
    ...
    {
        var (success, reader) = MultipartPayloadReader.TryCreate(Request.Body, Request.ContentType);
        if (!success)
        {
            return BadRequest($"Expected a multipart request, but got {Request.ContentType}");
        }

        var payloadId = await UploadPayloadsWith(reader!);
        var downloadUrl = FormatWithDownloadUrl(payloadId);
    ...
    {
        var payload = await _payloadPersistor.LoadPayload(id);
