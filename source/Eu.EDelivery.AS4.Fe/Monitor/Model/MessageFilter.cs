...
public class MessageFilter : BaseFilter<MessageEntity, Message>
{
    ...
    public IQueryable<TEntity> ApplyFilter<TEntity>(IQueryable<TEntity> query) where TEntity : MessageEntity
    {
        ...
        if (!string.IsNullOrEmpty(EbmsRefToMessageId))
        {
            ...
            {
                query = query.Where(qr => qr.EbmsRefToMessageId != null && qr.EbmsRefToMessageId.Contains(filter));
            }
            else if (EbmsRefToMessageId.EndsWith('*'))
            {
                query = query.Where(qr => qr.EbmsRefToMessageId != null && qr.EbmsRefToMessageId.StartsWith(filter));
            }
            else if (EbmsRefToMessageId.StartsWith('*'))
            {
                query = query.Where(qr => qr.EbmsRefToMessageId != null && qr.EbmsRefToMessageId.EndsWith(filter));
            }
            else
            {
        ...
        if (ContentType != null)
        {
            ...
            query = query.Where(qr => qr.ContentType != null && types.Any(data => qr.ContentType.StartsWith(data)));
        }

        if (Status != null && Status.Any())
        {
            List<string> statusStrings = [.. Status.Select(status => status.ToString())];
            query = query.Where(qr => qr.Status != null && statusStrings.Contains(qr.Status));
        }
        ...
        if (!string.IsNullOrEmpty(ActionName))
        {
            ...
            {
                query = query.Where(qr => qr.Action != null && qr.Action.Contains(filter));
            }
            else if (ActionName.EndsWith('*'))
            {
                query = query.Where(qr => qr.Action != null && qr.Action.StartsWith(filter));
            }
            else if (ActionName.StartsWith('*'))
            {
                query = query.Where(qr => qr.Action != null && qr.Action.EndsWith(filter));
            }
            else
            {
        ...
        if (!string.IsNullOrEmpty(Service))
        {
            ...
            {
                query = query.Where(qr => qr.Service != null && qr.Service.Contains(filter));
            }
            else if (Service.EndsWith('*'))
            {
                query = query.Where(qr => qr.Service != null && qr.Service.StartsWith(filter));
            }
            else if (Service.StartsWith('*'))
            {
                query = query.Where(qr => qr.Service != null && qr.Service.EndsWith(filter));
            }
            else
            {
        ...
        if (!string.IsNullOrEmpty(MPC))
        {
            ...
            {
                query = query.Where(qr => qr.Mpc != null && qr.Mpc.Contains(filter));
            }
            else if (MPC.EndsWith('*'))
            {
                query = query.Where(qr => qr.Mpc != null && qr.Mpc.StartsWith(filter));
            }
            else if (MPC.StartsWith('*'))
            {
                query = query.Where(qr => qr.Mpc != null && qr.Mpc.EndsWith(filter));
            }
            else
            {
