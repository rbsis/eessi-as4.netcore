...
{
    private readonly ILogger<RuntimeLoader> _logger;
    private static readonly Type[] _configuredAttributes =
    {
    ...
    /// <param name="folder">The settings folder.</param>
    private RuntimeLoader(ILogger<RuntimeLoader> logger, string folder)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(folder, nameof(folder));
        if (!Directory.Exists(folder))
        {
            throw new InvalidOperationException($"The module folder {folder} doesn't exist");
        }

        _logger = logger;

        var implementationsByInterface = LoadTypeImplementationsByInterfaceFromFiles(folder);
        ...
        MetaData = [.. CreateItemsForType<IPMode>(), CreateItemForType(typeof(Entities.SmpConfiguration))];
    }
    ...
    public static RuntimeLoader Initialize(ILogger<RuntimeLoader> logger, IOptions<ApplicationSettings> settings)
    {
        return new RuntimeLoader(logger, settings.Value.Runtime);
    }
    ...
    private Dictionary<Type, Type[]> LoadTypeImplementationsByInterfaceFromFiles(string folder)
    {
        Type[] interfaces =
        {
            typeof(IReceiver),
        ...
        var notConfigurableAttr = typeof(NotConfigurableAttribute);
    ...
    private Type[] LoadTypesFromAssemblyPath(string file)
    {
        try
        {
            return Assembly.LoadFile(Path.GetFullPath(file)).GetExportedTypes();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Load types from assembly failed: {AssemblyPath}", file);
            return [];
        }
    }
    ...
    private static ItemType CreateItemForType(Type type)
    {
        ...
        return new ItemType
        {
            ...
            Properties = [.. LoadPropertiesForType(type.Name.ToLower(), type)]
        };
    }

    private static IEnumerable<Property> LoadPropertiesForType(string parentPath, Type type)
    {
        IEnumerable<PropertyInfo> propertyInfos = [.. type.GetRuntimeProperties().Where(p => _configuredAttributes.Any(p.IsDefined))];
        ...
        return propertyInfos.Select(p =>
        {
            ...
            var childPath = parentPath + "." + p.Name.ToLower();
            return new Property
            {
                ...
                Path = childPath,
                Properties = [.. LoadPropertiesForType(childPath, p.PropertyType)]
            };
        });
    }
}
