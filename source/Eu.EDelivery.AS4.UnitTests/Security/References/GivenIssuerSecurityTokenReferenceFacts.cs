...
public class GivenIssuerSecurityTokenReferenceFacts
{
    ...
    public void ThenGetXmlContainsSecurityTokenReference()
    {
        ...
        var stubCertificate = certRepository.GetStubCertificate();

        // Act
        var xml = new IssuerSecurityTokenReference(stubCertificate).GetXml();
        ...
        var issuerName = xml.SelectEbmsNode("/dsig:X509Data/dsig:X509IssuerSerial/dsig:X509IssuerName");
        Assert.Equal(stubCertificate.IssuerName.Name, issuerName.InnerText);

        var serialNumberNode = xml.SelectEbmsNode("/dsig:X509Data/dsig:X509IssuerSerial/dsig:X509SerialNumber");
        var expectedSerialNumber = Convert.ToUInt64($"0x{stubCertificate.SerialNumber}", 16).ToString();
