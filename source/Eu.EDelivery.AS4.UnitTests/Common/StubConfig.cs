...
public class StubConfig : PseudoConfig
{
    ...
    public static readonly StubConfig Default = new();
    ...
    private static Dictionary<string, string> GetDefaultConfigSettings() => new(StringComparer.CurrentCultureIgnoreCase)
    {
        ["IdFormat"] = "{GUID}",
        ["Provider"] = "InMemory",
        ["ConnectionString"] = @"",
    ...
    public override string CertificateRepositoryType => typeof(StubCertificateRepository).AssemblyQualifiedName! ?? string.Empty;
    public override string DatabaseProvider => "InMemory";
    public override string DatabaseConnectionString => string.Empty;
    public override string? EbmsMessageIdFormat => GetSetting("IdFormat");
    ...
    {
        if (!_sendingPModes.TryGetValue(id, out var value))
        {
            return null;
        }
    ...
    public override IEnumerable<ReceivingProcessingMode> GetReceivingPModes()
    {
