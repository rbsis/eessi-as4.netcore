...
{
    private static readonly IEnumerable<object> Retries =
    [
        new NotifyRetry(
            currentRetryCount: 1,
            maxRetryCount: 3,
    ...
    private static readonly IEnumerable<object> Types =
    [
        new NotifyType<InMessage>(
            insertion: factory => (ebmsMessageId, current, max) =>
            {
                var m = new InMessage(ebmsMessageId) { Operation = Operation.ToBeNotified };
            ...
            {
                var rr = factory.GetRetryReliability(r => r.RefToInMessageId == entity.Id);
                return (rr?.CurrentRetryCount ?? 0, entity.Operation);
            }),
        new NotifyType<OutMessage>(
            insertion: factory => (ebmsMessageId, current, max) =>
            {
                var m = new OutMessage(ebmsMessageId) { Operation = Operation.ToBeNotified };
            ...
            operationGetter: (factory, entity) =>
            {
                var rr = factory.GetRetryReliability(r => r.RefToOutMessageId == entity.Id);
                return (rr?.CurrentRetryCount ?? 0, entity.Operation);
            }),
        new NotifyType<InException>(
            ...
            operationGetter: (factory, entity) =>
            {
                var rr = factory.GetRetryReliability(r => r.RefToInExceptionId == entity.Id);
                return (rr?.CurrentRetryCount ?? 0, entity.Operation);
            }),
        new NotifyType<OutException>(
            ...
            operationGetter: (factory, entity) =>
            {
                var rr = factory.GetRetryReliability(r => r.RefToOutExceptionId == entity.Id);
                return (rr?.CurrentRetryCount ?? 0, entity.Operation);
            })
    ];
