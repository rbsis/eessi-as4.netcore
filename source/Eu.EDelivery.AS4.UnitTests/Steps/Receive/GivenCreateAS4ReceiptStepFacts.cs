...
{
    [Property]
    public Property CreatesReceiptForEachUserMessage()
    {
        return Prop.ForAll(
            ...
            {
                // Arrange
                var fixture = AS4Message.Create(userMessages);
    ...
    [Fact]
    public async Task CreatesReceiptFromDefaultUserMessage()
    {
        // Arrange
        var messagingContext = CreateUserMessageWrappedInContext();

        // Act
        var result = await ExerciseCreateReceiptAsync(messagingContext);
    ...
    [Fact]
    public async Task CreatesReceiptWithoutNRRFormatForUnsignedUserMessage()
    {
        // Arrange
        var messagingContext = CreateUserMessageWrappedInContext();
    ...
    [Fact]
    public async Task CreatesReceiptWithNRRFormatForSignedUserMessage()
    {
        // Arrange
        var messagingContext = CreateSignedUserMessageWrappedInContext();
        messagingContext.ReceivingPMode!.ReplyHandling.ReceiptHandling.UseNRRFormat = true;
        messagingContext.ReceivingPMode!.ReplyHandling.ResponseSigning.IsEnabled = true;
    ...
    [Fact]
    public async Task CreatesReceiptWithoutNRRFormatIfSpecified()
    {
        // Arrange
        var messagingContext = CreateUserMessageWrappedInContext();
    ...
    [Fact]
    public async Task CreatesReceiptWithSameSignedReferencesTagsFromUserMessage()
    {
        // Arrange
        var messagingContext = CreateSignedUserMessageWrappedInContext();
        messagingContext.ReceivingPMode!.ReplyHandling.ReceiptHandling.UseNRRFormat = true;
        ...
        // Assert
        var receiptMessage = result.FirstSignalMessage as Receipt;
        var securityHeader = messagingContext.AS4Message?.SecurityHeader;
    ...
    {
        Assert.NotNull(receiptMessage.NonRepudiationInformation);
        var receiptRefs = receiptMessage.NonRepudiationInformation.MessagePartNRIReferences;

        Assert.All(
            securityHeader.GetReferences(),
            cryptoRef => Assert.Contains(receiptRefs, r => r.URI != null && r.URI.Equals(cryptoRef.Uri)));
    }

    [Fact]
    public async Task CreatesUnsignedReceiptFromUserMessage()
    {
        // Arrange
        var messagingContext = CreateUserMessageWrappedInContext();
    ...
    [Fact]
    public async Task CreatesMultihopReceiptIfReceivedUserMessageIsMultiHop()
    {
        // Arrange
        ...
        // Act
        var result = await ExerciseCreateReceiptAsync(fixture);
    ...
    [Fact]
    public async Task FailsToCreateNonRepudiationUnsignedReceipt()
    {
        // Arrange
        ...
        var sut = new CreateAS4ReceiptStep(NullLogger<CreateAS4ReceiptStep>.Instance, Default.IdentifierFactory);

        // Act
        var result = await sut.ExecuteAsync(fixture, CancellationToken.None);
    ...
    [Fact]
    public async Task FallbackToRegularReceiptWhenReferencedUserMessageIsntSigned()
    {
        // Arrange
