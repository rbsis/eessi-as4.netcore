...
{
    [Fact]
    public void ReturnsSameInstanceEvenWhenAcceptedOtherwise()
    {
        // Arrange
        var expectedUploader = Mock.Of<IAttachmentUploader>();
        var sut = new StubAttachmentUploaderProvider(expectedUploader);
        sut.Accept(condition: x => false, uploader: new StubAttachmentUploader(downloadUrl: string.Empty));

        // Act
        var actualUploader = sut.Get(type: null);
