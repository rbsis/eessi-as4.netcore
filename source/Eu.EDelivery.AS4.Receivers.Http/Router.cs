...
internal class Router : IRouter
{
    ...
    /// <returns></returns>
    public IRouter Via(IHttpGetHandler handler)
    {
    ...
    /// <param name="handler"></param>
    /// <returns></returns>
    public IRouter Via(IHttpPostHandler handler)
    {
    ...
    {
        ArgumentNullException.ThrowIfNull(httpContext.Request);
        ...
        try
        {
            if (request.HttpMethod == HttpMethod.Get.Method)
            {
                await _getHandlers
                    ...
                    .OrElse(() =>
                    {
                        _logger.LogDebug("Respond with 406 NotAccepted: no GET handler can handle this request");
            ...
            if (request.HttpMethod == HttpMethod.Post.Method)
            {
                ...
                using var agentResult = await prePostSelector(request);
                ...
                await _postHandlers
                    ...
                    .OrElse(() =>
                      {
                          _logger.LogDebug("Respond with 406 NotAccepted: no POST handler can handle this request");
            ...
            _logger.LogDebug("Respond with 405 MethodNotAllowed: request is neither GET or POST");
            await HttpResult
        ...
        {
            _logger.LogError(ex, "Route failed");
        }
    }
}
